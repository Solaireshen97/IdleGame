...
- 房间成员身份属于角色而不是用户；同一用户的不同角色可以分别加入不同房间，也可以同时加入同一个房间
- 同一角色在同一时刻仍只能占用一个房间
...
- `POST /api/battle/start` 需要登录后调用，推进当前激活角色在该房间中的一次战斗（当前角色未加入房间时会拒绝）
- `POST /api/battle/reset` 需要登录后调用，按当前激活角色的房间成员身份重置当前房间怪物血量
- `POST /api/battle/heal` 需要登录后调用，为当前激活角色在该房间中的角色恢复 10 HP
