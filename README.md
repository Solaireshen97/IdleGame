...
- 十个已装备栏位的武器攻击和最大 HP 均相加，不按属性筛选。角色现有的攻击与最大 HP 字段保存这个武器总和，天赋固定加成再叠加其上，因此战斗、角色信息和武器页面使用同一数值。换装不会恢复当前 HP；最大 HP 降低时会裁剪当前 HP。
- 老角色迁移时获得一把数值等同于原攻击和最大 HP 的主武器，原有战力不变。新角色按 `Weapons` 配置获得主武器和两把背包武器，方便试配；每把武器归角色本人，只有所属玩家能查看和操作。
- 武器盘页面展示主武器、3×3 副武器、属性和换装前后的基础总和。战斗进行期间锁定换装，主武器不能被直接卸空。武器被动技能、装备掉落将在后续阶段接入。
...
- 普通攻击、伤害技能和怪物反击共用后端计算器：`max(1, floor(max(1, (攻击 + 技能固定威力) × 攻击乘区 − 防御) × HP 状态乘区 × 暴击乘区 × 属性乘区 × 减伤乘区))`。每个乘区内部的百分比增减先相加，乘区之间相乘，最后统一向下取整。当前攻击、HP 状态、暴击乘区均为 100%；守护技能进入减伤乘区。后续武器被动和技能效果可以分别接入对应乘区。
