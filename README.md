...
- `POST /api/battle/start` 使用 `Authorization: ****** 推进当前用户所在房间成员的一次战斗（基于 roomId）
- `POST /api/battle/reset` 使用 `Authorization: ****** 重置当前房间怪物血量
- `POST /api/battle/heal` 使用 `Authorization: ****** 为当前用户在该房间中的角色恢复 10 HP
...
### 前端页面
- **房间列表页**（`/`、`/rooms`）：查看所有房间（仅展示基础信息），选择怪物类型，创建/删除/进入房间
