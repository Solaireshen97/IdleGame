...
const fights = JSON.parse(fs.readFileSync('docs/t1-combat-final.json', 'utf8')).Summary;
const trials = 2000;
...
const cases = [];
for (const region of world.Regions) for (const profession of ['knight', 'cleric']) {
  ...
  const field = samples.find(x => x.Key.Stage === 'field' && x.Key.Dungeon === normalDungeon.Code);
  ...
  const count = { hunts: Math.floor(budget.hunts * 3600 / field.MeanCycleSeconds),
  ...
  cases.push({ element: region.FeaturedElement, profession, count, exchangeCount,
    cyclesMinutes: { hunt: field.MeanCycleSeconds / 60, dungeon: weekDungeon.MeanCycleSeconds / 60, elite: weekElite.MeanCycleSeconds / 60 },
...
const report = { trials, budgetHours: budget, assumptions: [
  ...
  'Uses measured mean Lv7 hunt/featured dungeon/Lv10 elite times, preset field or week equipment and 100% victory for those targets.',
  ...
  'Quality distribution comes from production. Green-or-better counts any point allocation, not optimal rolls; combat week presets put the green point in skill one.',
